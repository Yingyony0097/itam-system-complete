...
function printHtml($title, $headers, $rows, $metaLines = []) {
    ...
    <body onload="window.print()">
        ...
        <p style="margin: 0; color: #555; font-size: 13px;">
            <?php echo e(tr('User:')); ?> <?php echo e($_SESSION['user_name'] ?? ''); ?> (<?php echo e($_SESSION['user_email'] ?? ''); ?>)
        </p>
...
switch ($type) {
    case 'my_assets':
        $assets = $assetModel->getByUser($userId);
        $headers = [tr('Asset Code'), tr('Name'), tr('Category'), tr('Serial Number'), tr('Status'), tr('Assigned Date'), tr('Purchase Date'), tr('Price')];
        $rows = [];
        foreach ($assets as $a) {
            $rows[] = [
                $a['asset_code'],
                ...
                $a['assigned_date'] ?: '-',
                $a['purchase_date'] ?: '-',
                number_format((float)($a['purchase_price'] ?? 0), 2)
            ];
        }
        $title = tr('My Assets Report');
        break;
    ...
    case 'my_activity':
        ...
        $headers = [tr('Date'), tr('Action'), tr('Asset'), tr('Performed By'), tr('Notes')];
        $rows = [];
        foreach ($logs as $l) {
            $rows[] = [
                $l['action_date'],
